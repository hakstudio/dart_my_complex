...
void main() {
  ...
  for (int i = 0; i < LENGTH; i++)
    listComplexCartesian.add(new Complex.cartesian(
        (random!.nextDouble() - 0.5) * 1000,
        (random!.nextDouble() - 0.5) * 1000));
  listComplexPolar = listComplexCartesian
      .map((Complex cc) => new Complex.polar(cc.modulus, cc.argument))
      .toList();
  newListComplexCartesian = listComplexPolar
      ?.map((Complex cp) => new Complex.cartesian(cp.real, cp.imaginary))
      .toList();
  listComplex
    ..addAll(listComplexCartesian)
    ..addAll(listComplexPolar!);
  ...
  test("transformation between polar and cartesian works", () {
    for (int i = 0; i < LENGTH; i++) {
      expect(listComplexPolar?[i].similar(newListComplexCartesian![i]),
          equals(true));
    }
  });
  ...
  test("round trip works", () {
    for (int i = 0; i < LENGTH; i++) {
      expect(listComplexCartesian[i].similar(newListComplexCartesian![i]),
          equals(true));
    }
  });
  ...
  test("subtraction works", () {
    for (int i = 0; i < LENGTH - 1; i++) {
      ...
      expect((cc1 - cc2).similar(cp1! - cp2!), equals(true),
          reason:
              "($cc1) - ($cc2) = (${cc1 - cc2}) not equal to \n($cp1) - ($cp2) = (${cp1 - cp2!})");
      expect((cp1 - cc2).similar(cc1 - cp2), equals(true),
          reason:
              "($cp1) - ($cc2) = (${cp1 - cc2}) not equal to \n($cc1) - ($cp2) = (${cc1 - cp2})");
      expect((cc1 - cc1).similar(cp1 - cp1), equals(true),
          reason:
              "($cc1) - ($cc1) = (${cc1 - cc1}) not equal to \n($cp1) - ($cp1) = (${cp1 - cp1})");
  ...
  test("multiplication works", () {
    for (int i = 0; i < LENGTH - 1; i++) {
      ...
      expect((cc1 * cc2).similar(cp1! * cp2!), equals(true),
          reason:
              "$cc1 * $cc2 (${cc1 * cc2}) should equal $cp1 * $cp2 (${cp1 * cp2})");
      expect((cp1 * cc2).similar(cc1 * cp2), equals(true));
      expect((cc1 * cc1).similar(cp1 * cp1), equals(true));
  ...
  test("my_complex constants", () {
    ...
    listComplex.forEach((Complex c) {
      [Complex.ONE, Complex.I, Complex.MINUSI, Complex.MINUSONE]
          .forEach((Complex c2) {
        a1 = c + c2;
        a2 = c2 + c;
        expect(a1.similar(a2), equals(true), reason: "+ $a1 = $a2");
        a1 = c - c2;
        a2 = c2 - c;
        expect(a1.similar(-a2), equals(true), reason: "- $a1 = - $a2");
        a1 = c * c2;
        a2 = c2 * c;
        expect(a1.similar(a2), equals(true), reason: "* ($c, $c2) $a1 = $a2");
        a1 = c / c2;
        a2 = c2 / c;
        expect(a1.similar(a2.inverse), equals(true), reason: "/ $a1 = 1/$a2");
      });
    });
  });
}
